...
- **Silent renew**: `automaticSilentRenew: true` — uses refresh tokens, not iframes. The `offline_access` scope is included so that SpacetimeAuth issues a refresh token at sign-in. Access tokens typically expire after 1 hour; the library automatically uses the refresh token to obtain a new access token before expiry. Without `offline_access` the IdP does not issue a refresh token, causing `oidc-client-ts` to attempt iframe-based renewal which fails in a PWA context (CSP / third-party cookie restrictions) and results in the user being logged out every hour.
